class Api {
     ...
     */
    static async query(token, searchParam) {
        const limit = 4;
        return fetch(`https://api.spotify.com/v1/search?q=name:${searchParam}&type=playlist&limit=${limit}`, {
            ...
            items === null || items === void 0 ? void 0 : items.forEach((item) => {
                let data = Object.assign(Object.assign({}, item), { image: item.images[0].url, author: item.owner.display_name });
                newItems.push(data);
            });
            return newItems;
        }).catch((err) => alert(err.message + " Пожалуйста, перезагрузите страницу"));
    }
    /**
     ...
     */
    static async followPlaylist(token, id) {
     ...
     */
    static async unfollowPlaylist(token, id) {
     ...
     */
    static async sendRequest(token, id, isFollow) {
...
Api.clientId = '44a8232cc5a34b68a8bfb31f0ae3200a';
Api.clientSecret = 'd390d8c6812c4d858d8cae2198c101c8';
export default Api;
